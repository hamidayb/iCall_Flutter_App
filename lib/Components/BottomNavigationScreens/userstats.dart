...
class _UserStatsState extends State<UserStats> {
  ...
  Future<void> convertToJson() async {
    ...
    List<AllCasesModel> myPosts = [];

    myMap.forEach((dynamic post) {
      AllCasesModel myPost = AllCasesModel.fromJson(post);
      if (myPost.category == "murder") {
        setState(() {
          murder = murder + 1;
        });
      } else if (myPost.category == "violence") {
        setState(() {
          violence = violence + 1;
        });
      } else if (myPost.category == "rape") {
        setState(() {
          rape = rape + 1;
        });
  ...
  Future readJsonFile() async {
    APIService.CasesDetails().then((response) {
      setState(() {
        CasesPosts = response;
      });
      if (response != "") {
